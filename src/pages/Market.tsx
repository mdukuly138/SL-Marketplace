    ...
    <div className="pb-4">
      ...
      <div className="px-4 pb-4 flex gap-2 overflow-x-auto no-scrollbar">
        <button
          ...
          className={cn(
            'shrink-0 rounded-pill px-3.5 py-1.5 text-xs font-semibold border',
            !activeCategory ? 'bg-ember text-base border-ember' : 'bg-surface text-muted border-border',
          )}
        >
          All
        </button>
        {categories.map((c) => (
          <button
            ...
            className={cn(
              'shrink-0 rounded-pill px-3.5 py-1.5 text-xs font-semibold border',
              activeCategory === c ? 'bg-ember text-base border-ember' : 'bg-surface text-muted border-border',
            )}
          >
            {c}
          </button>
      ...
      {filtered.length === 0 && (
        <p className="text-center text-muted text-sm mt-10">No listings match your search.</p>
      )}
    </div>
  )
}
